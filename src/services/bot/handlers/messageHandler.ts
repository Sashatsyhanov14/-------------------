...
export async function handleMessage(
    ...
    try {
        ...
        const rules = instructions && instructions.length > 0
            ? instructions.map((r: any) => `- ${r.rule_text}`).join("\n")
        ...
        const companyInfoStr = companyInfoRows && companyInfoRows.length > 0
            ? companyInfoRows.map((r: any) => `${r.key}: ${r.value}`).join("\n")
            : "";
        ...
        const agencyFiles = companyFiles && companyFiles.length > 0
            ? companyFiles.map((f: any) => `📄 ${f.name}${f.description ? ': ' + f.description : ''}`).join("\n")
            : "";

        const botKnowledge = `[КРИТИЧЕСКИЕ ИНСТРУКЦИИ]:
- МЫ ТОЛЬКО ПРОДАЕМ НЕДВИЖИМОСТЬ. НИКАКОЙ АРЕНДЫ.
        ...
        if (routerInstruction.instructions_for_manager_agent) {
            ...
            if (temp === "warm" || temp === "hot") {
                ...
                if (managers && managers.length > 0) {
                    ...
                    const notificationText = `🚀 <b>У вас новый лид!</b>\n\nКлиент: ${safeName}\nПерейдите в дашборд для просмотра деталей.`;
                    ...
                    for (const m of managers) {
                        ...
                        try {
                            await sendMessage(token, m.telegram_id, notificationText, {
                                ...
                                reply_markup: {
                                    inline_keyboard: [
                                        [{ text: "В дашборд ↗️", url: dashboardUrl }]
                                    ]
                                }
                            });
                        } catch (e) {
