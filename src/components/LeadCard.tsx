...
export default function LeadCard({ lead, onDetailsClick, onStatusUpdate, onDelete }: LeadCardProps) {
    ...
    const timeAgo =
        diffMinutes < 60
            ...
            : diffMinutes < 1440
                ...
                : `${Math.floor(diffMinutes / 1440)} д назад`;
    ...
    return (
        <div className={`group relative flex flex-col rounded-2xl overflow-hidden transition-all ${isInWork
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold text-neutral-100 flex items-center gap-2">
                                {flag} {lead.name || "Анонимный клиент"}
                                {/* Language badge */}
                                {langFlag && (
                                    <span className="text-sm" title={`Язык: ${clientLang?.toUpperCase()}`}>{langFlag}</span>
                                )}
                                {isInWork && (
                                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30">В работе ✓</span>
                                )}
                            </h3>
            ...
            {/* AI Client Summary */}
            {lead.data?.client_summary && (
                <div className="px-5 py-3 border-b border-neutral-800/50 bg-violet-500/5 transition-opacity">
            ...
            <div className="px-5 py-4 space-y-2 border-b border-neutral-800/50 bg-neutral-950/20">
                <div className="flex items-start gap-2 text-sm">
                    ...
                    <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest shrink-0 mt-0.5">ИНТЕРЕС:</span>
                ...
                <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-neutral-200 font-medium">
                        {lead.data?.budget ? `$${lead.data.budget.toLocaleString()}` : "Не определен"}
                    </span>
                </div>
                ...
                {lead.data?.urgency && (
                    <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">СРОКИ:</span>
                        <span className="text-neutral-300">{lead.data.urgency}</span>
                    </div>
                )}
                {lead.data?.purpose && (
                    <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-neutral-300">{lead.data.purpose}</span>
                    </div>
                )}
                {lead.data?.unit_type && (
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-neutral-500">🏢</span>
                        <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">ТИП:</span>
                        <span className="text-neutral-300">{lead.data.unit_type}</span>
                    </div>
                )}
                {lead.data?.preferred_areas && lead.data.preferred_areas.length > 0 && (
                    <div className="flex items-start gap-2 text-sm">
                        <span className="text-neutral-500 mt-0.5">📍</span>
                        <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mt-0.5">РАЙОН:</span>
                        <span className="text-neutral-300">{lead.data.preferred_areas.join(', ')}</span>
                    </div>
                )}
            </div>
                        ...
                        <div className="space-y-1.5 pl-6">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-semibold text-neutral-100">
                                    {focusUnit.name || "Объект"} <span className="text-xs text-neutral-500">(ID: {focusUnit.id.slice(0, 8)})</span>
                                </span>
                            </div>
            ...
            <div className="p-4 bg-neutral-950/20 flex items-center gap-2">
                ...
                <button
                    ...
                    title={isInWork ? "Снять с работы" : "В работу"}
                >
                    <Briefcase className="h-4 w-4" />
                    <span className="hidden sm:inline">{isInWork ? "В работе ✓" : "В работу"}</span>
                </button>
